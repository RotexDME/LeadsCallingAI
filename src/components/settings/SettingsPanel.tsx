...
export default function SettingsPanel() {
  ...
  const visibleConfig = config.filter(c => c.key !== 'AI_CONTEXT_PROMPT');
  const configuredCount = visibleConfig.filter(c => c.value.trim()).length;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            {configuredCount} of {visibleConfig.length} keys configured
            {sipConfigured
        ...
        <div className="space-y-6">
          {sortedGroups.map(group => {
            ...
            const entries = config
              .filter(c => c.group === group && c.key !== 'AI_CONTEXT_PROMPT')
              .sort((a, b) => {
                        ...
                        {isTextarea ? (
                          <>
                            ...
                            <p className="text-xs text-gray-600">
                              Defines the AI&apos;s role, tone, and behavioral rules applied on every call.
                            </p>
                          </>
                        ) : (
                          <>
